...
**这个 App 全程离线、不上架任何应用商店，APK 只能手动下载安装。三步：**

1. **下载** —— 点开 👉 **[NeighborLedger-v1.4.2.apk](NeighborLedger-v1.4.2.apk)**，在打开的文件页右上角点 **「Download」**（或「下载原始文件」）。
   - 手机浏览器可能提示「此类文件可能会损害您的设备」，选**仍要下载**。
   - 若用电脑下载，再把 apk 传到手机即可（微信文件传输助手 / 数据线都行）。
2. **安装** —— 在通知栏或「文件管理 → 下载」里点开这个 apk。
   - 小米 / 红米会拦一下：允许「文件管理」（或浏览器）**安装未知应用** → 点「继续安装」。
3. **开权限** —— 打开 App，按引导开启**通知使用权**；建议同时关闭电池优化、允许自启动。

**配套文件：**

- **[default_config.json](default_config.json)** —— **外置配置**（与 `app/src/main/assets/default_config.json` 同一份）。在 App 内导入即可改渠道开关 / 正则 / 人设文案，**不用重装**。
- **[安装到手机-打包安装步骤.txt](安装到手机-打包安装步骤.txt)** —— 更细的逐步说明 + 常见问题。
